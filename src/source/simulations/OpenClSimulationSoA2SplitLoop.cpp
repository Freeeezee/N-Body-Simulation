...
OpenClSimulationSoA2SplitLoop::OpenClSimulationSoA2SplitLoop(const float timeStep, const std::vector<Body>& bodies)
    : SimulationSoA2(timeStep, bodies) {
    initOpenCL();
    updateBuffers();
}

void OpenClSimulationSoA2SplitLoop::initOpenCL() {
    ...
    std::ifstream kernelFile("kernels/body-force-calculationSoASplitLoop.cl");
    if (!kernelFile) {
        throw std::runtime_error("Failed to open kernel file: kernels/body-force-calculationSoASplitLoop.cl");
    }
...
void OpenClSimulationSoA2SplitLoop::updateBuffers() {
    const size_t n = bodies.masses.size();
    const size_t floatSize = n * sizeof(float);
...
BodiesSoA2 OpenClSimulationSoA2SplitLoop::calculateNextTick() {
    ...
    size_t globalSize = static_cast<size_t>(numBodies);
    clEnqueueNDRangeKernel(queue, kernelForces, 1, nullptr, &globalSize, nullptr, 0, nullptr, nullptr);
...
OpenClSimulationSoA2SplitLoop::~OpenClSimulationSoA2SplitLoop() {
